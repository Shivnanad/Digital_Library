...
export async function getPurchasedBooks() {
  const res = await fetch(`${API}/orders/purchased`, {
    headers: authHeaders(),
  });
  const data = await res.json();
...
export async function checkBookPurchased(bookId) {
  const res = await fetch(`${API}/orders/purchased/${bookId}`, {
    headers: authHeaders(),
  });
  const data = await res.json();
...
export async function cancelOrderItem(orderId, bookId, reason, refundMethod, refundDetails = {}) {
  const res = await fetch(`${API}/orders/${orderId}/items/${bookId}/cancel`, {
    method: "POST",
    headers: authHeaders(),
